...
const { enable, orientation, location, active_monitor, monitor } =
  options.theme.osd;
...
const handleReveal = (self: any, property: string) => {
  ...
  Utils.timeout(DELAY, () => {
    ...
    if (count === 0) self[property] = false;
  });
};
  ...
  return Widget.Revealer({
    ...
    child: Widget.Box({
      ...
      children: orientation.bind("value").as((ort) => {
        if (ort === "vertical") {
          return [OSDLabel(ort), OSDBar(ort), OSDIcon(ort)];
        }

        return [OSDIcon(ort), OSDBar(ort), OSDLabel(ort)];
      }),
    }),
  });
};
