...
const getModulesForMonitor = (monitor: number, curLayouts: BarLayout) => {
  const foundMonitor = Object.keys(curLayouts).find(
...
const gdkMonitorIdToHyprlandId = (
  monitor: number,
  usedHyprlandMonitors: Set<number>,
): number => {
  ...
  const directMatch = hyprland.monitors.find((hypMon) => {
    ...
    return (
      gdkMonitor.key.startsWith(hyprlandKey) &&
      !usedHyprlandMonitors.has(hypMon.id) &&
      hypMon.id === monitor
    );
  });
  ...
  const fallbackMonitor = hyprland.monitors.find(
    (hypMon) => !usedHyprlandMonitors.has(hypMon.id),
  );
...
export const Bar = (() => {
  ...
  return (monitor: number) => {
    const hyprlandMonitor = gdkMonitorIdToHyprlandId(
      monitor,
          ...
          startWidget: Widget.Box({
            ...
            setup: (self) => {
              self.hook(layouts, (self) => {
                const foundLayout = getModulesForMonitor(
                  hyprlandMonitor,
                ...
                self.children = foundLayout.left
                  ...
                  .map(
                    (w) =>
                      widget[w](hyprlandMonitor) as Button<Gtk.Widget, unknown>,
                  );
              });
            },
          }),
