...
const { workspaces, monitorSpecific, workspaceMask, scroll_speed, spacing } =
...
const Workspaces = (monitor = -1) => {
  const currentMonitorWorkspaces = Variable(
    getCurrentMonitorWorkspaces(monitor),
  );
                ...
                child: Widget.Label({
                  ...
                  class_name: Utils.merge(
                    [
                      ...
                      options.bar.workspaces.numbered_active_indicator.bind(
                        "value",
                      ),
    ...
    return Widget.Box({
      children: Utils.merge(
        ...
        (
          ...
          const workspacesWithRules = Object.keys(workspaceRules).reduce(
            (acc: number[], k: string) => {
  ...
  return {
    component: Widget.Box({
      ...
      child: options.bar.workspaces.hideUnoccupied
        .bind("value")
        .as((hideUnoccupied) =>
          hideUnoccupied ? occupiedWses() : defaultWses(),
        ),
    }),
    isVisible: true,
    boxClass: "workspaces",
    props: {
      setup: (self: any) => {
        Utils.merge(
          [
            scroll_speed.bind("value"),
            options.bar.workspaces.hideUnoccupied.bind("value"),
          ],
