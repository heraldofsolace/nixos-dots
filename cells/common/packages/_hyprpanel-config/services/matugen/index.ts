...
export async function generateMatugenColors(): Promise<
  MatugenColors | undefined
> {
  ...
  try {
    ...
    const normalizedContrast =
      contrast.value > 1 ? 1 : contrast.value < -1 ? -1 : contrast.value;
    const contents = await bash(
      `matugen image ${wallpaperPath} -t scheme-${scheme_type.value} --contrast ${normalizedContrast} --json hex`,
    );

    return JSON.parse(contents).colors[
      options.theme.matugen_settings.mode.value
    ];
  } catch (error) {
...
export const replaceHexValues = (
  incomingHex: HexColor,
  ...
  const matugenVariation = getMatugenVariations(
    matugenColors,
    options.theme.matugen_settings.variation.value,
  );
  updateOptColor(
    matugenVariation.base,
    options.theme.bar.menus.menu.media.card.color as Variable<HexColor>,
  );
