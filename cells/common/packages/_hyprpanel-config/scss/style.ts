...
import {
  generateMatugenColors,
  replaceHexValues,
} from "../services/matugen/index";
...
function extractVariables(
  theme: typeof options.theme,
  prefix = "",
  matugenColors: MatugenColors | undefined,
) {
  ...
  for (let key in theme) {
    if (theme.hasOwnProperty(key)) {
      ...
      const replacedValue =
        isColor && matugenColors !== undefined
      ...
      if (
        ...
        if (typeof value.value !== "undefined") {
          ...
          result = result.concat(
            extractVariables(value, newPrefix, matugenColors),
          );
        }
