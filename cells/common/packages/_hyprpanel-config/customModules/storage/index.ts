...
export const Storage = () => {
  const storageModule = module({
    ...
    label: Utils.merge(
      [
        storageUsage.bind("value"),
        labelType.bind("value"),
        round.bind("value"),
      ],
      (
        storage: GenericResourceData,
        lblTyp: ResourceLabelType,
        round: boolean,
      ) => {
      ...
      setup: (self: Button<Gtk.Widget, Gtk.Widget>) => {
        inputHandler(self, {
          ...
          onScrollDown: {
            fn: () => {
              labelType.value = LABEL_TYPES[
                (LABEL_TYPES.indexOf(labelType.value) -
                  1 +
                  LABEL_TYPES.length) %
                  LABEL_TYPES.length
              ] as ResourceLabelType;
