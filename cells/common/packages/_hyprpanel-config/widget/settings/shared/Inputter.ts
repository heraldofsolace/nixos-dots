      ...
      switch (type) {
        ...
        case "string":
          return (self.children = [
            ...
            Widget.Entry({
              class_name: isUnsaved
                .bind("value")
                .as((unsaved) => (unsaved ? "unsaved" : "")),
        ...
        case "enum":
          return (self.child = EnumSetter(
            opt as unknown as Opt<string>,
            enums!,
          ));
        case "boolean":
          return (self.child = Widget.Switch({
            sensitive:
              disabledBinding !== undefined
                ? disabledBinding.bind("value").as((disabled) => !disabled)
                : true,
          })
            .on("notify::active", (self) => {
              ...
              if (
                ...
                !dependencies.every((d) => checkDependencies(d))
              ) {
                self.active = false;
                return;
              }
          ...
          return (self.child = Widget.Box({
            children: [
              ...
              Widget.Button({
                ...
                on_clicked: () => {
                  saveFileDialog(
                    exportData?.filePath as string,
                    exportData?.themeOnly as boolean,
                  );
                },
              }),
            ],
            ...
            setup: (self) =>
              self
                ...
                .on(
                  "font-set",
                  ({ font }) =>
                    (opt.value = font!.split(" ").slice(0, -1).join(" ") as T),
                ),
          }));
