...
export const filterConfigForNonTheme = (config: object) => {
  ...
  for (let key in config) {
    ...
    if (
      !(typeof config[key] === "string" && hexColorPattern.test(config[key]))
    ) {
      filteredConfig[key] = config[key];
    }
  }
...
export const saveFileDialog = (filePath: string, themeOnly: boolean): void => {
  ...
  const filterHexColorPairs = (jsonObject: object) => {
    ...
    for (let key in jsonObject) {
      if (
        ...
        hexColorPattern.test(jsonObject[key])
      ) {
  ...
  let filteredJsonObject = themeOnly
  ...
  dialog.set_current_name(
    themeOnly ? "hyprpanel_theme.json" : "hyprpanel_config.json",
  );
  ...
  if (response === Gtk.ResponseType.ACCEPT) {
    ...
    try {
      ...
      let outputStream = save_file.replace(
        null,
        false,
...
export const importFiles = (themeOnly: boolean = false): void => {
  ...
  if (response === Gtk.ResponseType.ACCEPT) {
    ...
    let [optionsSuccess, optionsContent] =
      optionsConfigFile.load_contents(null);
    ...
    let optionsConfig = JSON.parse(
      new TextDecoder("utf-8").decode(optionsContent),
    );
